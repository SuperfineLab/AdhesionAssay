...
if ~exist('ga_summary', 'var')
    ga_summary = table('Size', [0 9], ...
                       ...
                       'VariableNames', {'PlateID', 'SolveTime', 'OptimizedParameters', 'TotalError', 'RedChiSq', 'ExitFlag', 'GenerationSolveCount', 'MaxGenerations', 'PopulationSize'});
end
...
%     weights = rawdata.Weights;
    ...
    if sum(weights) == numel(weights), FuncTol = 3e-8; else FuncTol = 2e-10; end
...
%     [zlogforce_nN, mu, sigma] = zscore(logforce_nN(:));
    ...
    for k = 1:Nruns
        ...
        [optimized_params(k,:), error(k,1), exitflag(k,1), output(k,1)] = ga(@(p) objectiveFunction(p, fout.fcn, logforce_nN, fractionLeft, weights), ...
                                        fout.Nparams, fout.Aineq, fout.bineq, [], [], fout.lb, fout.ub, [], options);

%         optimized_params(k,:) = zoptimized_params(k,:) * sigma + mu;
        
        rchisq(k,:) = red_chisquare(optimized_params(k,:), fout.fcn, logforce_nN, fractionLeft, weights);
        
        t=toc;
        
        tmpga = table(PlateID, t, optimized_params(k,:), error(k,1), rchisq(k,:), exitflag(k,1), output.generations, max_generations, popsize, ...
                     'VariableNames', {'PlateID', 'SolveTime', 'OptimizedParameters', 'TotalError', 'RedChiSq', 'ExitFlag', 'GenerationSolveCount', 'MaxGenerations', 'PopulationSize'});
