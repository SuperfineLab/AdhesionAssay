...
%                  'ba_220902int25kpH7',    [adp, '2022.09.02__Int25k_pH7p4_assortedbeads']; ...
    ...
    lots_of_slashes = repmat({filesep},height(DataSrcs),1);
%     plateNames = DataSrcs(:,1);
    platepaths = [DataSrcs(:,2),lots_of_slashes];

    for i = 1:height(DataSrcs)
    platepaths{i,1} = [platepaths{i,1},platepaths{i,2}];
    end

    platepaths = platepaths(:,1);
